...
def main():
    setup_logging()
    logging.info("Starting ML-based NER evaluation")

    # Create results directory
    os.makedirs(os.path.join('results', 'ml', 'ner'), exist_ok=True)
    ...
    if treatment_mask.any():
        ...
        if treatment_df['attributes'].apply(lambda x: 'Status_Certainty' in x).any():
            X_attr = treatment_df['entity_text']
            y_attr = treatment_df['attributes'].apply(lambda x: x.get('Status_Certainty', 'Unknown'))

            report, accuracies = evaluate_attributes(
                models['Status_Certainty'],
                X_attr,
                y_attr,
                'Status_Certainty'
            )
    ...
    if response_mask.any():
        ...
        if response_df['attributes'].apply(lambda x: 'Certainty' in x).any():
            X_attr = response_df['entity_text']
            y_attr = response_df['attributes'].apply(lambda x: x.get('Certainty', 'Unknown'))
